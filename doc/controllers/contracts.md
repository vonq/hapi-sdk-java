...
```java
CompletableFuture<CreateContractResponseModel> createContractAsync(
...
PostContract body = new PostContract();
body.setChannelId(30.52);
body.setCredentials(com.vonq.api.marketplacesandbox.ApiHelper.deserialize("{\"key1\":\"val1\",\"key2\":\"val2\"}"));

contractsController.createContractAsync(xCustomerId, body).thenAccept(result -> {
...
```java
CompletableFuture<Void> deleteContractAsync(
...
contractsController.deleteContractAsync(contractId, xCustomerId).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<List<AutocompleteValuesResponseModel>> listAutocompleteValuesAsync(
...
[`List<AutocompleteValuesResponseModel>`](../../doc/models/autocomplete-values-response-model.md)
...
FacetAutocomplete body = new FacetAutocomplete();

contractsController.listAutocompleteValuesAsync(channelId, postingRequirementName, body).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<ListChannelsResponseModel> listChannelsAsync(
    final String search,
    final Integer limit,
...
| `acceptLanguage` | [`AcceptLanguageEnum`](../../doc/models/accept-language-enum.md) | Header, Optional | The language the client prefers. |
...
contractsController.listChannelsAsync(null, null, null, acceptLanguage).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<ListContractsResponseModel> listContractsAsync(
    final String xCustomerId,
    final Double limit,
    final Double offset)
```
...
contractsController.listContractsAsync(xCustomerId, null, null).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<ChannelModel> retrieveChannelAsync(
...
contractsController.retrieveChannelAsync(channelId, acceptLanguage).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<ContractModel> retrieveContractAsync(
    final String contractId,
    final String xCustomerId)
```
...
contractsController.retrieveContractAsync(contractId, xCustomerId).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
...
```java
CompletableFuture<MultipleContractsResponseModel> retrieveMultipleContractsAsync(
...
```java
List<String> contractsIds = new LinkedList<>();
contractsIds.add("contracts_ids7");
contractsIds.add("contracts_ids8");

contractsController.retrieveMultipleContractsAsync(contractsIds, null, null).thenAccept(result -> {
    // TODO success callback handler
}).exceptionally(exception -> {
    // TODO failure callback handler
    return null;
